...
export default function App() {
  ...
  const [authMode, setAuthMode] = useState<"login" | "signup" | "google_modal">("login");
  const [authForm, setAuthForm] = useState({ email: "", password: "", name: "", googleEmail: "" });
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(API_URL, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: authMode === "google_modal" ? "google_signin" : authMode,
          email: authMode === "google_modal" ? authForm.googleEmail : authForm.email,
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, background: "radial-gradient(circle at top right, rgba(251,191,36,0.1), transparent 40%), var(--bg-main)" }}>
        <div className="glass-panel card-gold-glow animate-fade-in" style={{ width: "100%", maxWidth: 420, padding: 32 }}>
          ...
          {authMode !== "google_modal" ? (
            ...
            <form onSubmit={handleAuthSubmit} className="animate-fade-in" style={{ display: "flex", flexDirection: "column", gap: 16, background: "rgba(255,255,255,0.05)", padding: 20, borderRadius: 16, border: "1px solid rgba(251,191,36,0.3)" }}>
              <div style={{ textAlign: "center" }}>
                <span style={{ fontSize: 32, display: "block", marginBottom: 8 }}>🌐</span>
                <h3 style={{ fontSize: 16, fontWeight: 900, color: "#fff" }}>Sign in with Google Account</h3>
                <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>For existing YoYo SMM users who signed up on our website using Google OAuth.</p>
              </div>
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "var(--text-muted)", marginBottom: 6, display: "block" }}>YOUR GOOGLE EMAIL</label>
                <input
                  type="email"
                  required
                  placeholder="name@gmail.com"
                  value={authForm.googleEmail}
                  onChange={(e) => setAuthForm({ ...authForm, googleEmail: e.target.value })}
                  className="input-field"
                />
              </div>
              <button type="submit" disabled={loading} className="btn-cyan" style={{ width: "100%", padding: 16, fontSize: 15 }}>
                {loading ? "Verifying Google Account..." : "Instant Google Sign In →"}
              </button>
              <button
                type="button"
                onClick={() => { setAuthMode("login"); triggerHaptic(); }}
          ...
          {authMode !== "google_modal" && (
            <>
              ...
              <button
                type="button"
                onClick={() => { setAuthMode("google_modal"); triggerHaptic(); }}
                style={{ width: "100%", padding: "14px", borderRadius: 14, fontSize: 14, fontWeight: 800, cursor: "pointer", border: "1px solid var(--border)", color: "#fff", background: "rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "center", gap: 12, transition: "all 0.2s" }}
              >
