...
export function classifyError(errMsg: string): ErrorClass {
  ...
  // Dead service or duplicate link lock on this provider — try fallback service ID / next panel
  if (
    ...
    e.includes("active order") || e.includes("wait until order") ||
    e.includes("duplicate") || e.includes("already exists") ||
    e.includes("link has active")
  ) return "next_service";
