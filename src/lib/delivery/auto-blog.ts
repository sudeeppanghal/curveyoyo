...
export async function generateOrderCaseStudy(orderId: string) {
  try {
    ...
    // Check if we already generated a blog for this order
    const existing = await prisma.blog.findFirst({
      where: { excerpt: { contains: `[Order: ${orderId}]` } }
    });
    if (existing) {
      console.log(`[AutoBlog] Case study for order ${orderId} already exists.`);
      return;
    }
...
Output ONLY valid JSON. Do not include markdown code block wraps (\`\`\`json ... \`\`\`).`;
    ...
    const blog = await prisma.blog.create({
      data: {
        title: data.title,
        slug: cleanSlug,
        excerpt: `${data.excerpt || "A real-time campaign performance study."} [Order: ${orderId}]`,
