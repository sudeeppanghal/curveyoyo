...
export async function checkAndRefillOrder(orderId: string): Promise<{
  ...
  let panels = order.user.panels;
  const platform = order.reel.platform.toLowerCase();
  if (platform === "instagram") {
    panels = panels.filter(p => p.apiUrl.includes("yoyomedia.in"));
  } else if (platform === "youtube" || platform === "facebook" || platform === "tiktok") {
    panels = panels.filter(p => p.apiUrl.includes("morethanpanel.com"));
  }
  if (!panels.length) {
    return { ok: false, error: `No active panels configured for platform ${platform}` };
  }
  const activePanel = panels[0]; // Primary platform panel
