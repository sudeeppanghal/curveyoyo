...
export function calculateEngagementDue(
  ...
  // How much engagement is NOW owed based on views progress (strictly capped at target)
  const owed = {
    likes:    Math.min(targets.likes,    Math.round(targets.likes    * fraction)),
    saves:    Math.min(targets.saves,    Math.round(targets.saves    * fraction)),
    shares:   Math.min(targets.shares,   Math.round(targets.shares   * fraction)),
    comments: Math.min(targets.comments, Math.round(targets.comments * fraction)),
    reposts:  Math.min(targets.reposts,  Math.round(targets.reposts  * fraction)),
  };

  // How much is still due (owed minus already sent, capped at total remaining target)
  const due = {
    likes:    Math.min(Math.max(0, targets.likes    - alreadyDelivered.likes),    Math.max(0, owed.likes    - alreadyDelivered.likes)),
    saves:    Math.min(Math.max(0, targets.saves    - alreadyDelivered.saves),    Math.max(0, owed.saves    - alreadyDelivered.saves)),
    shares:   Math.min(Math.max(0, targets.shares   - alreadyDelivered.shares),   Math.max(0, owed.shares   - alreadyDelivered.shares)),
    comments: Math.min(Math.max(0, targets.comments - alreadyDelivered.comments), Math.max(0, owed.comments - alreadyDelivered.comments)),
    reposts:  Math.min(Math.max(0, targets.reposts  - alreadyDelivered.reposts),  Math.max(0, owed.reposts  - alreadyDelivered.reposts)),
  };
  ...
  return {
    likes:    targets.likes    > 0 && alreadyDelivered.likes    < targets.likes    ? Math.max(0, due.likes    >= minLikes    || (isLastBatch && due.likes    > 0) ? due.likes    : 0) : 0,
    saves:    targets.saves    > 0 && alreadyDelivered.saves    < targets.saves    ? Math.max(0, due.saves    >= minSaves    || (isLastBatch && due.saves    > 0) ? due.saves    : 0) : 0,
    shares:   targets.shares   > 0 && alreadyDelivered.shares   < targets.shares   ? Math.max(0, due.shares   >= minShares   || (isLastBatch && due.shares   > 0) ? due.shares   : 0) : 0,
    comments: targets.comments > 0 && alreadyDelivered.comments < targets.comments ? Math.max(0, due.comments >= minComments || (isLastBatch && due.comments > 0) ? due.comments : 0) : 0,
    reposts:  targets.reposts  > 0 && alreadyDelivered.reposts  < targets.reposts  ? Math.max(0, due.reposts  >= minReposts  || (isLastBatch && due.reposts  > 0) ? due.reposts  : 0) : 0,
  };
}


export function evalOrganicSCurve(progress: number): number {
  const p = Math.max(0, Math.min(1, progress));
  if (p <= 0.0833) {
    const norm = p / 0.0833;
    return 0.0233 * Math.pow(norm, 1.8);
  } else if (p <= 0.3333) {
    const norm = (p - 0.0833) / (0.3333 - 0.0833);
    return 0.0233 + (0.2267 - 0.0233) * (0.3 * norm + 0.7 * Math.pow(norm, 1.4));
  } else if (p <= 0.6667) {
    const norm = (p - 0.3333) / (0.6667 - 0.3333);
    const smooth = norm * norm * (3 - 2 * norm);
    return 0.2267 + (0.7667 - 0.2267) * (0.85 * norm + 0.15 * smooth);
  } else if (p <= 0.8333) {
    const norm = (p - 0.6667) / (0.8333 - 0.6667);
    return 0.7667 + (0.9100 - 0.7667) * (1 - Math.pow(1 - norm, 1.5));
  } else {
    const norm = (p - 0.8333) / (1.0000 - 0.8333);
    return 0.9100 + (1.0000 - 0.9100) * (1 - Math.pow(1 - norm, 2.2));
  }
}
