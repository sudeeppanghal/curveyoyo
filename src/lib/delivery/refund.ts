...
export async function triggerMidwayRefund(orderId: string, isManual: boolean = false) {
  try {
    ...
    if (order.user.walletMode && !activePanel) {
    ...
    // If order.panelId is null, resolve the primary panel based on domain routing rules
    let activePanelId = order.panelId;
    if (!activePanelId) {
      const platUpper = (order.reel.platform ?? "").toUpperCase();
      const isMTP = 
        platUpper === "YOUTUBE" || 
        (platUpper === "FACEBOOK" && (order.likesTarget > 0 || order.priceCharged > 0)) ||
        (platUpper === "TIKTOK" && order.viewsTarget > 0);
      ...
      if (targetPanel) activePanelId = targetPanel.id;
    }
