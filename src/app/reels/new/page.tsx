...
export default function NewReelPage() {
  ...
  const [pacingUnit, setPacingUnit] = useState<"days" | "hours">("days");
  const [durationHoursVal, setDurationHoursVal] = useState(24);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const s = generateRawSchedule({
        totalViews: views,
        durationHours: pacingUnit === "hours" ? durationHoursVal : durationDays * 24,
  ...
  }, [views, durationDays, pacingUnit, durationHoursVal, style, engEnabled, likesOn, likesRatio, savesOn, savesRatio, sharesOn, sharesRatio, commentsOn, commentsRatio, repostsOn, repostsRatio]);
  ...
  const durationHours = pacingUnit === "hours" ? durationHoursVal : durationDays * 24;
  const curveInfo = CURVE_DESCRIPTIONS[style] || CURVE_DESCRIPTIONS["ORGANIC"] || { label: "Organic S-Curve", desc: "Natural viral growth — slow warmup, steady peak, smooth decay.", warmup: 4, peak: 8, icon: "🌅", category: "Classic", num: 6 };
  ...
  const minViewsRequired = Math.max(100, smmLimits.views?.min ?? 100);
  const maxViewsRequired = smmLimits.views?.max ?? 10000000;
  const canProceed2 = views >= minViewsRequired && views <= maxViewsRequired && (pacingUnit === "hours" ? durationHoursVal >= 1 : durationDays >= 1) && (!isCustomMode || !hasCustomScheduleErrors);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        router.push("/login?next=/reels/new");
        return;
      }
      let data: any = {};
      try {
  ...
  }, [reelUrl, platform, views, durationHours, style, curveInfo, engEnabled, likesOn, savesOn, sharesOn, commentsOn, repostsOn, likesRatio, savesRatio, sharesRatio, commentsRatio, repostsRatio, eng, router, saveAsTemplate, templateName, isCustomMode, customSchedule, selectedViewsService, durationDays, pacingUnit, durationHoursVal]);
  return (
    <div className="order-form-wrap" style={{ maxWidth: step === 2 ? 1100 : 640, width: "100%", margin: "0 auto", display: "flex", flexDirection: "column", gap: 24, transition: "max-width 0.3s ease-in-out" }}>
      <style>{`
        ...
        @media (max-width: 767px) {
          /* Main wrapper */
          .order-form-wrap { max-width: 100% !important; padding: 0 12px !important; }

          /* Step 2 two-column layout → single column */
          .order-step2-cols { display: grid !important; grid-template-columns: 1fr !important; gap: 16px !important; }
        ...
        <div style={{ borderRadius:24, padding:28, background:N.bg, boxShadow:N.raised, display:"flex", flexDirection:"column", gap:20, animation:"fadeUp 0.3s ease" }}>
          ...
          <div>
            ...
            {smmLimits.views && (
              ...
              {/* Pacing Unit Selector */}
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <label style={{ fontSize: 12, fontWeight: 800, color: "#fff" }}>Pacing Unit</label>
                <div style={{ display: "flex", background: "#0c0612", padding: 4, borderRadius: 10, border: "1px solid rgba(168, 85, 247, 0.15)" }}>
                  <button
                    type="button"
                    onClick={() => setPacingUnit("days")}
                    style={{
                      flex: 1,
                      padding: "8px",
                      borderRadius: 8,
                      ...
                      fontWeight: 800,
                      cursor: "pointer",
                      background: pacingUnit === "days" ? "rgba(168, 85, 247, 0.25)" : "transparent",
                      color: pacingUnit === "days" ? "#fff" : "#94a3b8",
                      transition: "all 0.2s"
                    }}
                  >
                    📅 Days
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 800,
                      cursor: "pointer",
                      background: pacingUnit === "hours" ? "rgba(168, 85, 247, 0.25)" : "transparent",
                      color: pacingUnit === "hours" ? "#fff" : "#94a3b8",
                      transition: "all 0.2s"
                    }}
                  >
                    ⏱️ Hours
              ...
              {pacingUnit === "days" ? (
                ...
                <Slider label="Duration (Hours)" value={durationHoursVal} min={1} max={168} step={1}
                  onChange={setDurationHoursVal} format={(v) => `${v} hour${v === 1 ? "" : "s"}`} />
              )}

              {/* Informational Message to user about safe pacing & zero failed orders feature */}
              <div style={{ padding: 14, borderRadius: 14, background: "rgba(168,85,247,0.1)", border: "1px solid rgba(168,85,247,0.25)", color: "#c084fc", display: "flex", flexDirection: "column", gap: 4, fontSize: 12, fontWeight: 600 }}>
                <span style={{ fontWeight: 800 }}>✨ Smart-Queue Fallback Enabled</span>
                <span>We check the completion status of each batch. If a previous batch is still running on the SMM API, the system automatically delays the next batch and reschedules the remaining batches to prevent concurrent order blockages and ensure zero failed orders!</span>
              </div>

              {views / (pacingUnit === "hours" ? durationHoursVal / 24 : durationDays) > 5000 && (
          ...
          {isCustomMode ? (
            ...
            <div style={{marginTop:8}}>
              ...
              />
              <div className="order-step2-cols" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginTop:16}}>
            ...
            <div className="order-step2-cols" style={{display:'grid',gridTemplateColumns:'1.2fr 1fr',gap:32,alignItems:'start',marginTop:8}}>
