...
export async function GET(request: NextRequest) {
  ...
  // Support ?limit=N for dashboard (default 100, max 100)
  const url = new URL(request.url);
  const limitParam = parseInt(url.searchParams.get("limit") || "100", 10);
  const take = Math.min(Math.max(1, limitParam), 100);
...
export async function POST(request: NextRequest) {
  try {
    // Rate limit: 20 order requests per minute per IP
    const ip = request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "anonymous";
    const ipAllowed = await checkRateLimit(ip, 20);
    if (!ipAllowed) {
      return NextResponse.json(
        { error: "Too many orders placed. Please wait a minute and try again." },
        { status: 429 }
      );
    }
  ...
  if (views < 100 || views > 10_000_000) {
    return NextResponse.json({ error: "Minimum order is 100 views. Please increase your views count." }, { status: 400 });
  }
  ...
  // - More Than SMM (morethanpanel.com) is ONLY for: YouTube views/likes, Facebook likes, TikTok Views
  // - YoyoMedia (yoyomedia.in) is strictly for everything else (Instagram, TikTok likes/comments, other Facebook services)
  const isMoreThanService = 
    ...
    (platform.toUpperCase() === "TIKTOK" && views > 0);
  ...
  // 1. Determine if the main service ordered goes to MoreThanPanel or YoyoMedia
  const mainIsMTP = 
    platform.toUpperCase() === "YOUTUBE" || 
    (platform.toUpperCase() === "FACEBOOK" && viewsType === "likes") ||
    (platform.toUpperCase() === "TIKTOK" && viewsType === "views");

  const targetDomain = mainIsMTP ? "morethanpanel.com" : "yoyomedia.in";

  // Filter routedPanels based on the target provider domain
  const providerPanels = routedPanels.filter(p => p.apiUrl.toLowerCase().includes(targetDomain));

  // Find all panels in the target provider pool that have mapped services
  const validAdminPanels: { panel: any; services: any[] }[] = [];
  const searchPanels = providerPanels.length > 0 ? providerPanels : routedPanels;
  ...
  // To make pricing 100% consistent, select the highest priority panel (index 0) from the target provider pool
  // rather than selecting a random panel.
  ...
  const selected = adminPool[0]; // Highest priority panel
  const adminPanel = selected.panel;
  const adminServices = selected.services;
  primaryPanel = adminPanel;

  const getServiceRateInfo = async (svcType: string) => {
    // If the selected primary panel maps the service, use it!
    const directSvc = adminServices.find(x => x.type === svcType);
    if (directSvc) {
      return { customRate: directSvc.customRate, originalRate: directSvc.originalRate };
    }
    
    // Otherwise, resolve the fallback panel according to the routing rules
    const isMTP = 
      platform.toUpperCase() === "YOUTUBE" || 
      (platform.toUpperCase() === "FACEBOOK" && svcType === "likes") ||
      (platform.toUpperCase() === "TIKTOK" && svcType === "views");
      
    const targetPanel = await prisma.panel.findFirst({
      where: {
        userId: null,
        isActive: true,
        status: { not: "OFFLINE" },
        apiUrl: { contains: isMTP ? "morethanpanel.com" : "yoyomedia.in" }
      },
      orderBy: { priority: "asc" }
    });

    if (targetPanel) {
      const svc = await prisma.adminService.findFirst({
        where: { panelId: targetPanel.id, platform: platform.toUpperCase() as any, type: svcType }
      });
      if (svc) {
        return { customRate: svc.customRate, originalRate: svc.originalRate };
      }
    }
    
    const defaultCustomRates: Record<string, number> = {
      views: 3.0, reach_impressions_views: 3.0, likes: 5.0, saves: 5.0, shares: 8.0, comments: 15.0, reposts: 12.0, followers: 15.0, subscribers: 25.0
    };
    const defaultOriginalRates: Record<string, number> = {
      views: 0.6, reach_impressions_views: 0.6, likes: 1.0, saves: 1.0, shares: 1.6, comments: 3.0, reposts: 2.4, followers: 3.0, subscribers: 5.0
    };
    return {
      customRate: defaultCustomRates[svcType] ?? 5.0,
      originalRate: defaultOriginalRates[svcType] ?? 1.0
    };
  };

  const viewsRateKey = (platform === "INSTAGRAM" && viewsType === "reach_impressions_views") ? "reach_impressions_views" : "views";

  const rateInfoViews = await getServiceRateInfo(viewsRateKey);
  const rateInfoLikes = await getServiceRateInfo("likes");
  const rateInfoSaves = await getServiceRateInfo("saves");
  const rateInfoShares = await getServiceRateInfo("shares");
  const rateInfoComments = await getServiceRateInfo("comments");
  const rateInfoReposts = await getServiceRateInfo("reposts");

  const viewsCost = (views / 1000) * rateInfoViews.customRate;
  const likesCost = (engTargets.likesTarget / 1000) * rateInfoLikes.customRate;
  const savesCost = (engTargets.savesTarget / 1000) * rateInfoSaves.customRate;
  const sharesCost = (engTargets.sharesTarget / 1000) * rateInfoShares.customRate;
  const commentsCost = (engTargets.commentsTarget / 1000) * rateInfoComments.customRate;
  const repostsCost = (engTargets.repostsTarget / 1000) * rateInfoReposts.customRate;

  totalPrice = parseFloat((viewsCost + likesCost + savesCost + sharesCost + commentsCost + repostsCost).toFixed(2));

  const viewsOrigCost = (views / 1000) * rateInfoViews.originalRate;
  const likesOrigCost = (engTargets.likesTarget / 1000) * rateInfoLikes.originalRate;
  const savesOrigCost = (engTargets.savesTarget / 1000) * rateInfoSaves.originalRate;
  const sharesOrigCost = (engTargets.sharesTarget / 1000) * rateInfoShares.originalRate;
  const commentsOrigCost = (engTargets.commentsTarget / 1000) * rateInfoComments.originalRate;
  const repostsOrigCost = (engTargets.repostsTarget / 1000) * rateInfoReposts.originalRate;

  const totalOriginalCost = parseFloat((viewsOrigCost + likesOrigCost + savesOrigCost + sharesOrigCost + commentsOrigCost + repostsOrigCost).toFixed(2));
  ...
  if (Array.isArray(customSchedule) && customSchedule.length > 0) {
    ...
    // Trigger SEO Case Study generation immediately on order placement (fire-and-forget)
    import("@/lib/delivery/auto-blog").then(({ generateOrderCaseStudy }) => {
      generateOrderCaseStudy(order.id).catch(err => console.error("[Order Creation] Auto-blog trigger error:", err));
    }).catch(err => console.error("[Order Creation] Auto-blog import error:", err));
  }
  ...
  const profit = parseFloat((totalPrice - totalOriginalCost).toFixed(2));
  const msg = `🚀 **New Order Placed!**\n\n` +
    `👤 **User:** \`${dbUser.email}\`\n` +
    `🌐 **Platform:** \`${platform}\`\n` +
    `🎯 **Targets:**\n` +
    `  • Views: \`${views.toLocaleString()}\`\n` +
    `  • Likes: \`${engTargets.likesTarget.toLocaleString()}\`\n` +
    `  • Comments: \`${engTargets.commentsTarget.toLocaleString()}\`\n` +
    `  • Shares: \`${engTargets.sharesTarget.toLocaleString()}\`\n` +
    `  • Saves: \`${engTargets.savesTarget.toLocaleString()}\`\n` +
    `  • Reposts: \`${engTargets.repostsTarget.toLocaleString()}\`\n\n` +
    `💵 **Price Charged (Custom 5x):** \`₹${totalPrice.toLocaleString()}\`\n` +
    `📉 **SMM Provider Cost (Actual):** \`₹${totalOriginalCost.toLocaleString()}\`\n` +
    `💰 **Platform Profit:** \`₹${profit.toLocaleString()}\`\n\n` +
  ...
  sendTelegramAlert(msg.replace(/\*\*/g, "*")).catch(console.error);
  sendDiscordAlert("orders", msg).catch(console.error);
