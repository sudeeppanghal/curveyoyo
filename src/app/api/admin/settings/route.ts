...
export async function PATCH(request: NextRequest) {
  ...
  const { trc20Address, bep20Address, priceUsdt, siteName, freeTrialHours, maintenanceMode, supportEmail, upiId, upiQrCode, minDeposit, apifyKeys, mtpMode, mtpApiKey } = body;
  ...
  const settings = await prisma.adminSettings.upsert({
    ...
    update: {
      ...
      ...(mtpMode !== undefined && { mtpMode: Boolean(mtpMode) }),
      ...(mtpApiKey !== undefined && { mtpApiKey }),
    },
  });
