...
export async function GET(
  ...
  const chartData = order.deliveryEvents.map((e) => {
    ...
    return {
      ...
      likes:    fired?.likes    ?? custom?.likes    ?? (isExecuted ? 0 : plannedLikes),
      saves:    fired?.saves    ?? custom?.saves    ?? (isExecuted ? 0 : plannedSaves),
      shares:   fired?.shares   ?? custom?.shares   ?? (isExecuted ? 0 : plannedShares),
      comments: fired?.comments ?? custom?.comments ?? (isExecuted ? 0 : plannedComments),
      reposts:  fired?.reposts  ?? custom?.reposts  ?? (isExecuted ? 0 : plannedReposts),
...
export async function PATCH(
  ...
  if (action === "cancel") {
    return NextResponse.json({ error: "Cancellation is disabled." }, { status: 403 });
  }
