...
import { sendTelegramAlert } from "@/lib/telegram"; // forwards to Discord
  ...
  try {
    ...
    if (action === "login") {
      ...
      if (!dbUser) {
        ...
        try {
          sendDiscordAlert("system",
            `🚨 **New YoYo SMM App User (Signup)**\n\n` +
            `👤 **Name:** ${dbUser.name}\n` +
            `📧 **Email:** ${dbUser.email}\n` +
    ...
    if (action === "place_order") {
      const {
        ...
        curveStyle = "SLOW_START", durationHours = 1,
      ...
      const isMoreThanService = 
        ...
        (platform.toUpperCase() === "FACEBOOK" && (likesRatioPct > 0 || views > 0)) ||
      ...
      if (routedPanels.length === 0) {
        return NextResponse.json({ ok: false, error: "Service temporarily unavailable for this platform." }, { status: 503, headers: corsHeaders });
      }
      ...
      const onlineAdminPanels = validPanels.filter(x => x.panel.status !== "OFFLINE");
      const adminPool = onlineAdminPanels.length > 0 ? onlineAdminPanels : validPanels;
      ...
      const getOriginalRate = (type: string, fallback: number) => {
        const s = selected.services.find(x => x.type === type);
        return s ? s.originalRate : fallback;
      };
      ...
      const viewsOrigCost = (views / 1000) * getOriginalRate("views", 0.6);
      const likesOrigCost = engagementEnabled ? ((views * (likesRatioPct / 100)) / 1000) * getOriginalRate("likes", 1.0) : 0;
      const savesOrigCost = engagementEnabled ? ((views * (savesRatioPct / 100)) / 1000) * getOriginalRate("saves", 1.0) : 0;
      const sharesOrigCost = engagementEnabled ? ((views * (sharesRatioPct / 100)) / 1000) * getOriginalRate("shares", 1.6) : 0;
      const commentsOrigCost = engagementEnabled ? ((views * (commentsRatioPct / 100)) / 1000) * getOriginalRate("comments", 3.0) : 0;
      const repostsOrigCost = engagementEnabled ? ((views * (repostsRatioPct / 100)) / 1000) * getOriginalRate("reposts", 2.4) : 0;
      ...
      const msg = `🚀 **New Order via User Mobile App!**\n\n` +
        `👤 **User:** \`${dbUser.email}\`\n` +
        `🌐 **Platform:** \`${platform}\`\n` +
        `🎯 **Targets:**\n` +
        `  • Views: \`${views.toLocaleString()}\` (${curveStyle})\n` +
        `  • Likes: \`${(engagementEnabled ? Math.round(views * (Number(likesRatioPct) / 100)) : 0).toLocaleString()}\`\n` +
        `  • Comments: \`${(engagementEnabled ? Math.round(views * (Number(commentsRatioPct) / 100)) : 0).toLocaleString()}\`\n` +
        `  • Shares: \`${(engagementEnabled ? Math.round(views * (Number(sharesRatioPct) / 100)) : 0).toLocaleString()}\`\n` +
        `  • Saves: \`${(engagementEnabled ? Math.round(views * (Number(savesRatioPct) / 100)) : 0).toLocaleString()}\`\n` +
        `  • Reposts: \`${(engagementEnabled ? Math.round(views * (Number(repostsRatioPct) / 100)) : 0).toLocaleString()}\`\n\n` +
        `💵 **Price Charged (Custom 5x):** \`₹${totalPrice.toFixed(2)}\`\n` +
        `📉 **SMM Provider Cost (Actual):** \`₹${totalOriginalCost.toFixed(2)}\`\n` +
        `💰 **Platform Profit:** \`₹${profit.toFixed(2)}\`\n\n` +
    ...
    if (action === "submit_crypto_deposit") {
      ...
      sendDiscordAlert("billing",
        ...
        `🔗 **TXID:** \`${cleanTxHash}\`\n` +
        `⏳ **Status:** Pending Verification\n` +
        `🔗 **Admin Panel:** ${process.env.NEXT_PUBLIC_APP_URL || "https://yoyosmm.online"}/admin`,
