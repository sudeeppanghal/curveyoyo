...
function LoginForm() {
  ...
  const getInitialError = () => {
    ...
    const raw = decodeURIComponent(searchParams.get("msg") || "");
    if (raw.includes("AuthPKCECodeVerifierMissingError") || raw.includes("PKCE") || raw.includes("verifier")) {
      return "Google sign-in session expired. Please sign in with your email and password below.";
    }
    if (raw.includes("Invalid email or password")) {
      return "Invalid email or password. Please check and try again.";
    }
    return raw || "Google sign-in failed. Please sign in with email and password below.";
  };
  ...
  useState(() => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        // Save user email locally for client components
        if (typeof window !== "undefined") {
          localStorage.setItem("user_email", form.email.toLowerCase().trim());
        }
      ...
      if (authError) {
        setError(formatError(data?.error) || "Invalid email or password. Please check and try again.");
        return;
      }
      if (typeof window !== "undefined") {
        localStorage.setItem("user_email", form.email.toLowerCase().trim());
    ...
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"24px 16px", background:N.bg, fontFamily:"'Inter',-apple-system,sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .neo-btn-alt:hover{box-shadow:8px 8px 16px #c8d0e7,-8px -8px 16px #ffffff !important}
      `}</style>
      ...
      <div style={{ width:"100%", maxWidth:420 }}>
        ...
        <div style={{ borderRadius:24, padding:"32px 28px", background:N.bg, boxShadow:N.raised }}>
          ...
          <button onClick={handleGoogle} disabled={googleLoading} className="neo-btn-alt"
