...
export default function UpdatePasswordPage() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        // Listen to auth state changes (handles URL hash fragments automatically)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError) {
        setError(authError.message || "Failed to update password. Please request a new reset link.");
        return;
      }
        ...
        {success ? (
          ...
          <>
            ...
            <div style={{ borderRadius:24, padding:"32px 28px", background:N.bg, boxShadow:N.raised }}>
              ...
              {verifying ? (
                <div style={{ textAlign: "center", padding: "20px 0", color: N.muted, fontSize: 13, fontWeight: 700 }}>
                ...
                <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:18 }}>
                  <div>
                    ...
                    <NeoInput type="password" placeholder="Min 6 characters" value={password} onChange={setPassword} required />
                  </div>
              ...
              <p style={{ textAlign:"center", fontSize:13, color:N.muted, marginTop:22, fontWeight:600 }}>
                ...
                <Link href="/forgot-password" style={{ color:N.accent, textDecoration:"none", fontWeight:800 }}>Request link →</Link>
              </p>
            </div>
          </>
        )}
