...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Fetch user's recent orders to check for active campaigns
    // Only fetch last 30 orders for the dashboard (not 100) for speed
    fetch("/api/orders?limit=30")
      .then((r) => r.json())
      .then((d) => {
        const list = d.orders ?? [];
        const runningList = list.filter((o: any) => o.status === "DELIVERING" || o.status === "QUEUED" || o.status === "PAUSED");
        // Cap at 5 concurrent status fetches to avoid overwhelming the server
        const toFetch = runningList.slice(0, 5);
        if (toFetch.length > 0) {
    ...
    // Fetch panels — show immediately from DB, then update health in background
    fetch("/api/panels")
      .then((r) => r.json())
      .then((d) => {
        ...
        setPanels(loaded);
        if (loaded.length > 0) {
          // ⚡ NON-BLOCKING: fire health check in background, DO NOT await
          // Pings external SMM APIs and can take 30-60s — never block render
          fetch("/api/panels/health", { method: "POST" })
            .then(res => res.ok ? res.json() : null)
            .then(healthData => {
              if (!healthData) return;
              setPanels(prev => prev.map(p => {
                const match = healthData.panels?.find((r: any) => r.id === p.id);
                return match ? { ...p, status: match.status, balance: match.balance, currency: match.currency } : p;
              }));
            })
            .catch(() => {}); // silently ignore health check errors
        }
      })
      .catch(() => {});
  }, []);
      ...
      {announcement && (announcement.title || announcement.imageUrl) && (
        <div style={{ display: "flex", justifyContent: "center", width: "100%", marginBottom: 20 }}>
          <a
            ...
            target={announcement.targetLink?.startsWith("http") ? "_blank" : undefined}
            rel={announcement.targetLink?.startsWith("http") ? "noopener noreferrer" : undefined}
            onClick={(e) => {
              if (!announcement.targetLink) {
                e.preventDefault();
              }
            }}
            style={{ textDecoration: "none", width: "100%", maxWidth: 1025, cursor: announcement.targetLink ? "pointer" : "default", display: "block" }}
          >
            <div
              className="promo-card-animated"
              style={{
                borderRadius: 24,
