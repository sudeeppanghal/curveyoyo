...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const onTicketsPage = pathname === "/tickets" || pathname.startsWith("/tickets/");
  const visibleNav = isSuspended ? NAV.filter(n => n.href === "/tickets") : NAV;
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: 18, fontWeight: 900, marginBottom: 10, color: "#dc2626" }}>Account Suspended</h2>
          <p style={{ fontSize: 13, color: N.muted, fontWeight: 600, lineHeight: 1.6, marginBottom: 24 }}>
            Your account has been suspended. Please contact support to resolve this.
          </p>
          <Link href="/tickets" className="neo-btn" style={{
            display:"flex",
            alignItems:"center",
            justifyContent:"center",
            gap:8,
    ...
    <div style={{ minHeight:"100vh", display:"flex", background:N.bg, fontFamily:"'Inter',-apple-system,sans-serif", color:N.text }}>
      ...
      <aside className="dash-sidebar" style={{
        ...
        <nav style={{ flex:1, overflowY:"auto", padding:"14px 12px", display:"flex", flexDirection:"column", gap:4 }}>
          ...
          {visibleNav.map(({ href, label, icon, external }: any) => {
            const isExternal = external || href.startsWith("http");
            const active = !isExternal && (pathname === href || (href !== "/dashboard" && pathname.startsWith(href)));
            if (isExternal) {
              return (
                <a key={href} href={href} target="_blank" rel="noopener noreferrer" className="nav-link" style={{
                  display:"flex", alignItems:"center", gap:11, padding:"10px 12px", borderRadius:12,
                  fontSize:12, fontWeight:800, textDecoration:"none",
                  color:"#eab308", background:"rgba(234,179,8,0.06)", border:"1px dashed rgba(234,179,8,0.3)",
                  transition:"all 0.2s", margin:"2px 0"
                }}>
                  <span style={{ color:"#eab308", display:"flex" }}>{icon}</span>
                  <span style={{ whiteSpace:"nowrap" }}>{label}</span>
                  <span style={{ marginLeft:"auto", fontSize:9, fontWeight:900, background:"#eab308", color:"#000", padding:"2px 5px", borderRadius:4, textTransform:"uppercase" }}>50% OFF</span>
                </a>
              );
            }
            return (
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
        ...
        <a
          ...
          style={{
            ...
            gap: 8,
            padding: "7px 16px",
            background: "linear-gradient(90deg, #d97706, #ea580c, #b91c1c, #d97706)",
            color: "#ffffff",
            fontSize: 12,
            fontWeight: 800,
            textDecoration: "none",
        ...
        >
          <span style={{ background: "rgba(255,255,255,0.25)", padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 900 }}>🔥 SPECIAL OFFER</span>
          <span>Get 50% Cheaper Services than YoyoSMM!</span>
          <span style={{ textDecoration: "none", fontWeight: 900, background: "#fff", color: "#ea580c", padding: "2px 10px", borderRadius: 8, fontSize: 11, marginLeft: 4 }}>Claim 50% Off at YTCPM.com →</span>
        </a>
        ...
        <header className="mob-topbar" style={{
          ...
          {/* Logo */}
          <Link href={isSuspended ? "/tickets" : "/dashboard"} style={{ display:"flex", alignItems:"center", gap:9, textDecoration:"none" }}>
